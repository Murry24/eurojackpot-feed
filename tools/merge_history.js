...
async function main() {
  ...
  // base history (vybudovaná build_history.js z CSV)
  const base = await readJsonSafe(HISTORY, { draws: [] });

  // latest (fetch.js)
  const latest = await readJsonSafe(FEED, { draws: [] });

  // merge podľa dátumu (posledný vyhráva)
  const map = new Map();
  for (const d of base.draws || [])   map.set(d.date, d);
  for (const d of latest.draws || []) map.set(d.date, d);

  const merged = Array.from(map.values()).sort(byDateDesc);

  await fs.writeFile(HISTORY, JSON.stringify({ draws: merged }, null, 2), "utf8");
